...
* Ghostty
#+BEGIN_SRC conf :tangle ~/.config/ghostty/config.ghostty :mkdirp yes
  ...
  keybind = "ctrl+shift+k=scroll_to_top"
  keybind = "ctrl+shift+j=scroll_to_bottom"

  keybind = "ctrl+a>c=clear_screen"
...
* Direnv
#+BEGIN_SRC sh :tangle ~/.config/direnv/direnvrc :mkdirp yes
  layout_mamba() {
...
* sqlite
#+BEGIN_SRC sh :tangle ~/.config/sqlite3/sqliterc :mkdirp yes
  .mode columns
  .header on
#+END_SRC
...
#+begin_src conf :tangle ~/.config/systemd/user/emailproxy.service :mkdirp yes
  ...
  [Service]
  Type=simple
  ExecStart=%h/environment/local/bin/micromamba run -r %h/environment/local/micromamba/ -n local_environment emailproxy --config-file %h/.config/emailproxy.cfg --no-gui --external-auth
  TimeoutStopSec=20
  Restart=on-failure
