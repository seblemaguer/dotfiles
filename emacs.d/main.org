...
** Folding
For the folding, I am relying on outline-minor mode and more specifically outline-magic.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package yaml-mode
    ...
    (yaml-mode . (lambda ()
                   (outline-minor-mode)
                   (setq outline-regexp "^ *\\([-] \\)?\\([\"][^\"]*[\"]\\|[a-zA-Z0-9_-]*\\): *\\([>|]\\|&[a-zA-Z0-9_-]*\\)?$"))))
...
#+begin_src emacs-lisp
  (use-package magit
    :ensure t
    :tags '("VC")
    :commands (magit hydra-magit/body)
    :hydra
...
#+begin_src emacs-lisp
  (use-package alert
    ...
    (defun alert-sauron-notify (info)
      (sauron-add-event (if (plist-get info :title)
                            (if (symbolp (plist-get info :title))
                                (plist-get info :title)
                              (intern (plist-get info :title)))
                          (if (plist-get info :category)
...
#+begin_src emacs-lisp
  (use-package mu4e
    ...
    :config
    (setq mu4e-maildir "~/mail"                  ;; Directory of my mails
          mu4e-change-filenames-when-moving t    ;;
          mu4e-compose-dont-reply-to-self t      ;; Don't reply to myself
          mu4e-attachment-dir "~/Downloads"

          ;; User interface
          mu4e-view-use-gnus t                   ;; Use gnus buffer view
          mu4e-view-show-images t                ;; Show images in the html rendering
          mu4e-view-show-addresses t             ;; Show the URL
          mu4e-confirm-quit nil                  ;; Do not ask to quit !
          )
...
#+begin_src emacs-lisp
  (defhydra hydra-flycheck (:pre (progn (setq hydra-hint-display-type t) (flycheck-list-errors))
