...
#+BEGIN_SRC emacs-lisp :tangle ~/.emacs.d/init.el
  ...
  (defvar slm~machine-not-connected '("lemagues-laptop")
    "Hostname of computers which are not connected to
    internet. This variable is needed to ignore some package which
    requires some synchronization (dropbox, ....).")
...
#+begin_src emacs-lisp :tangle ~/.emacs.d/init.el
  (use-package prodigy
    :ensure quelpa
    :unless (member (system-name) slm~machine-not-connected)
