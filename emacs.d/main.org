...
#+begin_src emacs-lisp
  ...
  ;; Exclusion
  (add-to-list 'recentf-exclude (format "%s/Dropbox/org/.*" (getenv "HOME")))
  ...
  (add-to-list 'recentf-exclude (format "%s/\\.elfeed/index" (getenv "HOME")))
...
#+BEGIN_SRC emacs-lisp
  (use-package js2-mode
    ...
    ;; Subpackage
    (use-package js-auto-beautify :ensure t)
    ...
    (add-hook 'js2-post-parse-callbacks
              (lambda ()
                (let ((buf (buffer-string))
                      (index 0))
                  ((when )hile (string-match "\\(goog\\.require\\|goog\\.provide\\)('\\([^'.]*\\)" buf index)
                   ((save-excursion )tq index (+ 1 (match-end 0)))
...
#+BEGIN_SRC emacs-lisp
  (use-package nxml-mode
    ...
    ;; Helper to format
    (defun xml-format ()
      "Using xmllint to correctly format the selected region"
      (interactive)
      (save-excursion
    ...
    (add-hook 'org-finalize-agenda-hook
              (lambda ()
                (save-excursion
                  ;; MSP calendar
                  (goto-char (point-min))
                  (while (re-search-forward "Calendar-MSP:" nil t)
                    (add-face-text-property (line-beginning-position) (+ 1 (point-at-eol))
                                            '(:weight bold :background "Tomato")))

                  ;; MSP hiwis
                  (goto-char (point-min))
                  (while (re-search-forward "Calendar-MSP-part-timers:" nil t)
                    (add-face-text-property (line-beginning-position) (+ 1 (point-at-eol))
                                            '(:weight bold :background "DeepSkyBlue")))

                  ;; MSP hiwis
                  (goto-char (point-min))
                  (while (re-search-forward "Calendar-studio:" nil t)
                    (add-face-text-property (line-beginning-position) (+ 1 (point-at-eol))
                                            '(:weight bold :background "yellow")))

                  ;; Aikido
                  (goto-char (point-min))
                  (while (re-search-forward "Calendar-Aikido:" nil t)
                    (add-face-text-property (line-beginning-position) (+ 1 (point-at-eol))
                                            '(:weight bold :background "yellow")))

                  ;; Personal
                  (goto-char (point-min))
                  (while (re-search-forward "Personnal:" nil t)
                    (add-face-text-property (line-beginning-position) (+ 1 (point-at-eol))
                                            '(:weight bold :background "green")))))))
#+end_src
** Capturing
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (use-package org-projectile
    ...
    ;; We add each todo from each repo to the org-agenda-files
    (setq org-projectile:per-repo-filename "todo.org"
...
** Continuous building
#+begin_src emacs-lisp
  (use-package build-status :ensure t)
  (use-package travis
    :ensure t
    :config
...
#+begin_src emacs-lisp
  (use-package indent-guide
    ...
    (add-to-list 'indent-guide-inhibit-modes 'emms-playlist-mode))
#+end_src
*** Column/line
#+begin_src emacs-lisp
  (use-package nlinum
...
** Information
#+begin_src emacs-lisp
  (defhydra yt-hydra/help (:color blue :hint nil :idle 0.4 global-map "<f1>")
    "
                                                                       ╭───────────────┐
                                                                       │ Documentation │
        ╭──────────────────────────────────────────────────────────────┴───────────────╯

          [_f_] function documentation
          [_v_] variable documentation
        --------------------------------------------------------------------------------
    "
    ("f" describe-function)
    ("v" describe-variable))
