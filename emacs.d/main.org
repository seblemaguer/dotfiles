...
#+begin_src emacs-lisp
  (use-package alert
    ...
    (defun alert-adapt-mode-line (info)
      (message (princ info))
      (message (format "%d <= %d, %d?"
                       ;; (cdr (assq (plist-get info :severity) alert-highlight-priorities))
                            alert-highlight-threshold alert-highlight-current-level 3))
    ...
    (setq alert-default-style 'mode-line-alert2)
...
#+begin_src emacs-lisp
  (defhydra hydra-main (:color teal :hint nil)
   ...
   Org. related             Help                Zooming          processes
  ------------------------------------------------------------------------------------------
  _c_: org-capture        _f_: function doc.    _+_: zoom in     _p_: prodigy
