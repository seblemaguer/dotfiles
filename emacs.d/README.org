...
#+OPTIONS: num:nil p:nil pri:nil stat:nil tags:t tasks:t tex:t timestamp:t
#+OPTIONS: toc:nil
#+OPTIONS: todo:nil |:t
#+SELECT_TAGS: export
...
#+transclude: [[file:~/.emacs.d/tangle.el]]  :lines 3-69 :src emacs-lisp
...
#+transclude: [[file:~/.emacs.d/tangle.el]]  :lines 73-80 :src emacs-lisp
...
:PROPERTIES:
:ID:       04bbb5be-60e1-46a1-a1d5-f531a0477f84
:END:
...
#+begin_src emacs-lisp
  (use-package use-package-tags
    :ensure (:wait t :protocol ssh :type git :host github :repo "seblemaguer/use-package-tags")
    :config
    (when no-connection
...
:PROPERTIES:
:ID:       aaa3f2c1-4981-4076-8d50-7ba41e1e529a
:END:
...
:END:
All my passwords are stored in a keepassxc database. I am relying on keepass-mode which itself relies on =keepassxc-cli=.

#+begin_src emacs-lisp
  (if (executable-find "keepassxc-cli")
      (use-package keepass-mode
        :ensure (:protocol ssh :type git :host github :repo "seblemaguer/keepass-mode")
        :tags '("CORE" "PASSWORDS")
        :mode ("\\.kdbx$")
        :commands (keepass-mode-get))

    (use-builtin keepass-mode-auth-source
        :tags '("CORE" "PASSWORDS")
        :mode ("\\.kdbx$")
        :commands (keepass-auth-source-enable)
        :init
        (setq auth-sources `(,keepass-db))
        (keepass-auth-source-enable))

    (warn "keepassxc-cli is not available. No passwords will be accessible")
  )
#+END_SRC
...
#+begin_src emacs-lisp
  ...
  (use-package sauron
    :ensure (:wait t :protocol ssh :type git :host github :repo "seblemaguer/sauron" :branch "functional")
    :tags '("CORE" "NOTIFICATION")
...
#+transclude: [[file:keytable_summary.org::float-narrow-indirect-entry-keybindings]]
...
#+begin_src emacs-lisp
  ...
  (use-package float-narrow-indirect
    :ensure (:host github :repo "yibie/float-narrow-indirect")
    :tags '("CORE" "NAVIGATION")
    :bind (
      <<keybindings-gen(table=float-narrow-indirect-entry-keybindings)>>
    )

    :config
    (setq fni-floating-window-size         '(0.3 . 0.5) ;; Size of floating windows (width . height) as ratio of parent frame
          fni-floating-frame-border-color  "#ecf0f1" ;; Border color for floating frames
          fni-floating-frame-transparency  '(95 . 90) ;; Transparency for floating frames (active . inactive)
          fni-buf-name-prefix              "NI-"
...
** DONE Security
CLOSED: [2026-07-24 Fri 15:49]
Since emacs 30, all buffers are untrusted by default.
=trust-manager= facilitates the management of buffers to be trusted or not.
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (org-after-todo-state-change . toggle-ordered-tag)
    ...
    :init
    (add-to-list 'safe-local-variable-values '(org-latex-reference-command . "\\cref{%s}"))
    (defun toggle-ordered-tag ()
      "Helper to toggle the \"ordered\" tag when the \"ORDERED\" todo state is activated."
      (let* ((tags (org-get-tags nil t)))
        (if (string= org-state "ORDERED")
            (when (not (member "ordered" tags))
              (setq tags (add-to-list 'tags "ordered")))
          (setq tags (delete "ordered" tags)))
        (org-set-tags tags)))
    ...
    (defun org-set-beamer-note ()
      "Helper to just the set the BEAMER_env property to NOTE."
      (org-set-property "BEAMER_env" "NOTE"))
    ...
    (define-minor-mode org-highlight-translation-mode
      "Helper mode to highlight translated word marked using square brackets."
      :group 'org
      (if org-highlight-translation-mode
          (highlight-regexp "\\[[a-zA-Z ?'-]+\\]" 'font-lock-string-face)
        (unhighlight-regexp "\\[[a-zA-Z ?'-]+\\]"))))
...
:PROPERTIES:
:ID:       2ed7c319-f712-411a-8f2e-df3426074b34
:END:
=lte= is an helper to edit long tables in the buffer indirection way.
However, it causes it issue with the way I use =org-noted=, so for now it is disabled
...
*** DONE Highlight some parts of a table
CLOSED: [2025-10-01 Wed 09:28]
:PROPERTIES:
:ID:       d4596499-289c-4830-b58a-88c0f3637e03
:END:
=org-table-highlight= provide a neat way to visualise some columns, rows or cells of org tables.
...
*** DONE Drawing
CLOSED: [2026-09-03 Thu 14:26]
It is now possible to draw on a browser and save the image to be inserted in an org file.
...
#+begin_src emacs-lisp
  (use-builtin ox-html ;; org-contrib
    :after ox
    :tags '("ORG")
    :commands (org-html-export-to-html org-html-export-as-html)
    :custom
    ...
    (add-to-list 'org-file-apps '("\\.x?html?\\'" . "helium %s")))
...
#+begin_src emacs-lisp
  (use-builtin ox-latex ;; org-contrib
    ...
    (defun ox-latex-env-adjust-levels (tree backend info)
      "Promote headlines nested inside LATEX_ENV headlines."
      (org-element-map tree 'headline
        (lambda (hl)
          (let ((promote 0)
                (parent (org-element-property :parent hl)))
            ;; Count enclosing LATEX_ENV headlines.
            (while parent
              (when (and (eq (org-element-type parent) 'headline)
                         (org-element-property :LATEX_ENV parent))
                (cl-incf promote))
            ...
            (when (> promote 0)
              (org-element-put-property
               hl :level
               (max 1 (- (org-element-property :level hl)
                         promote)))))))
        tree)
    (add-to-list 'org-export-filter-parse-tree-functions
                 #'ox-latex-env-adjust-levels)

    (defun ox-latex-env (orig headline contents info)
      "Converts any headline with :LATEX_ENV: property to an environment.
    To control the environment, you can add options in the property :LATEX_ENV_OPT:"
      (let ((env (org-element-property :LATEX_ENV headline))
            (options (org-element-property :LATEX_ENV_OPT headline)))
        (if env
    ...
    (advice-add 'org-latex-headline :around #'ox-latex-env))
  ...
  (use-builtin org-gradle-export
    :tags '("ORG" "GRADLE" "LATEX")
    :load-path default-plugin-dir
    :commands (org-gradle-export-beamer-student org-gradle-export-beamer-teacher org-gradle-export-beamer)
    :custom
    (org-gradle-export-nb-workers 1)
    (org-gradle-export-log-level 'info))
#+end_src
...
*** DONE Habit / Recurrent tasks
CLOSED: [2026-09-01 Tue 13:52]
#+BEGIN_SRC emacs-lisp
...
#+transclude: [[file:keytable_summary.org::org-clock-convenience-keybindings]]
  ...
  (use-package org-clock-convenience
    ...
    :bind (
      :map org-agenda-mode-map
       <<keybindings-gen(table=org-clock-convenience-keybindings)>>
    ))
#+end_src
...
#+transclude: [[file:keytable_summary.org::olivetti-entry-keybindings]]
...
#+begin_src emacs-lisp
  (use-package olivetti
    ...
    :bind (
      <<keybindings-gen(table=olivetti-entry-keybindings)>>
    )

    :hook
    (Info-mode . olivetti-mode)
    (ewww-mode . olivetti-mode)
...
#+begin_src emacs-lisp
  ...
  (use-package pdffontetc
    :after pdf-tools
    :commands (pdffontetc-display-combined-metadata-and-font-info)
    :tags '("DOCUMENT" "DESKTOP"))
#+end_src
...
#+transclude: [[file:keytable_summary.org::jinx-entry-keybindings]]

#+begin_src emacs-lisp
  (use-package jinx
    :tags '("CORE" "WRITING" "HELPERS")
    :bind (
      <<keybindings-gen(table=jinx-entry-keybindings)>>
    ))
#+end_src
...
#+begin_src emacs-lisp
  (use-package org-roam
    ...
    (org-directory cloud-org-root-dir)
  ...
  (use-package org-mem
    ...
    :after org-roam
    :custom
    (org-mem-watch-dirs (list cloud-org-root-dir))
    (org-mem-do-sync-with-org-id t)
    (org-roam-db-update-on-save nil)
    (org-mem-roamy-do-overwrite-real-db t)

    :hook
    (org-mem-post-full-scan-functions . #'my-set-agenda-files)
  ...
  (use-package org-roam-project-dashboard
    ...
    :init
    (defun org-roam-db-sync-advice (&rest _args) (org-roam-db-sync))
    (advice-add 'org-roam-project-dashboard-show :before 'org-roam-db-sync-advice))
#+end_src
...
#+begin_src emacs-lisp
  (use-package org-noter
    ...
    (defun org-noter-transfer-to-rm ()
      ...
      (interactive)
      (let ((pdf-file (org-entry-get (point) "NOTER_DOCUMENT")))
        (if (and pdf-file (file-exists-p pdf-file))
            (shell-command (format "micromamba run -n local_environment rm_import -t / \"%s\"" pdf-file))
...
*** DONE Zotero
CLOSED: [2026-01-08 Thu 15:27]
:PROPERTIES:
:ID:       6f306296-f0a3-4548-aab5-2a3c968274fe
:END:
I personally tend to avoid Zotero as it is just yet-another overhead, but some colleagues like it so I try to play nicely (sometimes :D).
To authenticate, my pivate configuration defines the helper =set-zotero-token=.
...
#+begin_src emacs-lisp
  ...
  (use-package undo-fu
    ...
    (undo-outer-limit  (* 10 1024 1024))) ;; 10 mb
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package kirigami
    :ensure (kirigami :type git :host github :repo "jamescherti/kirigami.el")
    :tags '("FOLDING")
    :bind (
        <<keybindings-gen(table=kirigami-entry-keybindings)>>
    ))

  (use-package occult
    :tags '("FOLDING")
    :bind (
        <<keybindings-gen(table=occult-entry-keybindings)>>
    ))
#+end_src
...
#+transclude: [[file:keytable_summary.org::corfu-doc-entry-keybindings]]
#+transclude: [[file:keytable_summary.org::cape-entry-keybindings]]
...
#+begin_src emacs-lisp
  ...
  (use-package corfu-doc
    ...
    :bind (
      :map corfu-map
      <<keybindings-gen(table=corfu-doc-entry-keybindings)>>
    ))
  ...
  (use-package cape
    :tags '("COMPLETION")
    :bind (
        <<keybindings-gen(table=cape-entry-keybindings)>>
    )
    :init
...
#+begin_src emacs-lisp
  (use-package consult
    ...
    (consult-customize consult-ripgrep consult-git-grep consult-grep consult-man
...
** DONE Filling
CLOSED: [2026-02-19 Thu 16:33]
:PROPERTIES:
:ID:       165ec710-dab6-44a5-a66e-6d951d586a59
:END:
I am tuned to semantic filling, so using =fancy-fill-paragraph= enables this in a pretty neat way.
...
I also shamelessly use the helpers provided in by Andrey Listopadov in https://andreyor.st/posts/2024-03-05-accepting-your-own-solutions-to-your-own-problems/.
...
#+begin_src emacs-lisp
  ...
  (use-package treesit-langs
    :ensure (:fetcher github :repo "emacs-tree-sitter/treesit-langs" :wait t)
    :custom (treesit-langs-bundle-version "0.12.329")
    :when (treesit-p)
    :demand t
    :preface
    (cl-defun treesit-install-and-remap (lang &key url revision source-dir modes remap org-src)
  ...
  a list of modes to remap to a symbol REMAP."
      (when (and (fboundp 'treesit-available-p)
        ...
        (unless (treesit-language-available-p lang)
            (add-to-list 'treesit-language-source-alist (list lang url revision source-dir))
...
#+begin_src emacs-lisp
  ...
  (use-builtin eglot
    ...
    (unless (string= (system-name) work-hostname)
      (setq native-comp-compiler-options '("-march=znver3" "-Ofast" "-g0" "-fno-finite-math-only"
                                           "-fgraphite-identity" "-floop-nest-optimize" "-fdevirtualize-at-ltrans"
                                           "-fipa-pta" "-fno-semantic-interposition" "-flto=auto" "-fuse-linker-plugin")
            native-comp-driver-options '("-march=znver3" "-Ofast" "-g0" "-fno-finite-math-only"
                                         "-fgraphite-identity" "-floop-nest-optimize" "-fdevirtualize-at-ltrans"
                                         "-fipa-pta" "-fno-semantic-interposition" "-flto=auto" "-fuse-linker-plugin")))
    ...
    (add-to-list 'eglot-server-programs
                 '((qml-mode qml-ts-mode) . ("qml-language-server")))
    (add-to-list 'eglot-server-programs
  ...
  (use-package flyover
    ...
    :custom-face
    (flyover-error   ((t :background "#FFCCCB"
                         :foreground "#000000"
                         :height 0.9
                         :weight normal)))

    (flyover-warning ((t :background "#FDBA84"
                         :foreground "#000000"
                         :height 0.9
                         :weight normal)))

    (flyover-info    ((t :background "#CCFFCB"
                         :foreground "#000000"
                         :height 0.9
                         :weight normal))))
#+end_src
...
#+transclude: [[file:keytable_summary.org::dape-entry-keybindings]]
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package dape
    ...
    :bind (
      :map dape-global-map
      <<keybindings-gen(table=dape-entry-keybindings)>>
    )
...
*** DONE HDF5
CLOSED: [2025-07-29 Tue 10:05]
:PROPERTIES:
:ID:       3cb58d48-5718-4f41-b1a5-bb2c5dee2f37
:END:

For python binary files, I tend to prefer the use of the hdf5 format.
=hdf5-viewer= provides an easy way to inspect the files.

#+BEGIN_SRC emacs-lisp
  (use-package hdf5-viewer
    :ensure (hdf5-viewer :type git :host github :repo "hdf5-viewer/hdf5-viewer")
    :tags '("PROGRAMMING" "DATA")
    :config
...
#+transclude: [[file:keytable_summary.org::python-tdd-entry-keybindings]]
...
#+begin_src emacs-lisp
  (use-builtin python
    ...
    :init
    (treesit-install-and-remap 'python
  ...
  (use-package python-tdd-mode
    :ensure (python-tdd-mode :type git :host github :repo "seblemaguer/python-tdd-mode" :branch "fix_global_conflict")
    :tags '("PROGRAMMING" "PYTHON")
    :hook (python-ts-mode . python-tdd-mode)
    :bind (
      :map python-tdd-mode-command-map
      <<keybindings-gen(table=python-tdd-entry-keybindings)>>
    )

    :config
    (setq python-tdd-mode-test-runner 'pytest
          python-tdd-mode-auto-run-on-save nil
          python-tdd-mode-scroll-output t
          python-tdd-mode-buffer-popup t
          python-tdd-mode-blink-enabled nil))

  (use-package ein
    :tags '("PROGRAMMING" "PYTHON" "IDE")
    :commands (ein:run)
    :custom-face
    (ein:codecell-input-area-face      ((t :background "#E8F9FF")))
    (ein:markdowncell-input-area-face  ((t :background "#E8FFF0")))
    (ein:cell-output-area              ((t :background "#FFEDE8"))))

  (use-package pydoc
    :tags '("PROGRAMMING" "PYTHON" "DOCUMENTATION")
    :commands (pydoc pydoc-at-point))
#+end_src
...
:PROPERTIES:
:ID:       e4d77518-ef58-4fdb-88dc-afda651efffe
:END:
#+BEGIN_SRC emacs-lisp
  (use-package qml-ts-mode
    ...
    :when (treesit-p)
    :config
    (treesit-install-and-remap 'qmljs
...
** DONE Databases [100%]
:PROPERTIES:
:ID:       8f949156-6838-4ce0-841d-a55759802795
:END:
=clutch= is a SQL client. I am mainly using it with sqlite, but it supports postgres & mysql which I am also using sporadically.
...
#+BEGIN_SRC emacs-lisp
  (use-package clutch
    ...
    (defun clutch-insert-sqlite-table-list-query ()
      """Helper to generate and insert the query to list all the tables in a SQLite databse."""
      (interactive)
      (insert "SELECT name FROM sqlite_schema WHERE type ='table' AND name NOT LIKE 'sqlite_%';")))
...
*** DONE Overleaf
CLOSED: [2026-07-24 Fri 15:53]
:PROPERTIES:
:ID:       95bb6b9f-c354-47a8-96a8-f8459ee93bce
:END:
As I am using overleaf to write academic papers (and actually everything else).
As a result, I am using 2 packages: =overleaf.el= to do live editing, =git-overleaf.el= for local editing.
...
#+begin_src emacs-lisp
  (use-package overleaf
    ...
    (setq overleaf-cookies `((".overleaf.com"
                              ,(format "overleaf_session2=%S"
                                       (keepass-get-password keepass-db "overleaf-api"))
                              ,(string-to-number (keepass-get-attribute keepass-db
                                                                        "overleaf-api"
                                                                        "expiry_time")))))
  ...
  (use-package git-overleaf
    ...
    (let ((cookie-file "~/.overleaf-cookies.gpg"))
      (setopt git-overleaf-auth-backend 'firefox-cookies
              git-overleaf-cookie-storage "~/.overleaf-cookies.gpg")))
#+end_src
...
#+begin_src emacs-lisp
  (use-builtin bibtex
    ...
    (defun bibtex-tidy-buffer ()
      "Apply bibtex-tidy to clean and sort the current bibtex buffer."
      (interactive)
      (let ((cmd (format "bibtex-tidy %s %s %s --v2 -m %s"
                         (shell-quote-argument (buffer-file-name))
                         "--curly --numeric --space=4 --align=13 --no-escape --enclosing-braces=title,journal,publisher"
...
Luckily, =markdown-mode= provide more than enough (and it requires =edit-indirect= to be installed to edit code blocks).
...
#+begin_src emacs-lisp
  ...
  (use-package markdown-indent-mode
    :tags '("WRITING" "MARKDOWN")
    :hook (md-ts-mode . markdown-indent-mode))
...
:END:
As I have the luck to deal with multiple languages (French, English, Finnish and Bulgarian), I need to have a quick access to some translations and dictionaries.
My main point of entry is =glosbe= but sometimes need to go a bit more detailed.
For this, I am using google-translate via the package =go-translate=.
...
#+begin_src emacs-lisp
  (use-package glosbe
    ...
    :bind (
      <<keybindings-gen(table=glosbe-translate-entry-keybindings)>>
    )
    :custom
    ...
    :config
    (defun glosbe-translate-word-fi (word)
      "Helper to find the translation of an Finnish `word' in English."
      (interactive
       (list (read-string (format "Word [%s]: " (thing-at-point 'word)) (thing-at-point 'word))))
      (glosbe-translate-word "fi" "en" word))


    (defun glosbe-translate-word-en (word)
      "Helper to find the translation of an English `word' in Finnish."
      (interactive
       (list (read-string (format "Word [%s]: " (thing-at-point 'word)) (thing-at-point 'word))))
      (glosbe-translate-word "en" "fi" word)))
...
#+begin_src emacs-lisp
  ...
  (use-package magit-prime
    :tags '("VC" "GIT")
    :hook
    (magit-pre-refresh . magit-prime-refresh-cache))
...
#+transclude: [[file:keytable_summary.org::project-x-entry-keybindings]]
#+transclude: [[file:keytable_summary.org::projection-entry-keybindings]]
...
#+begin_src emacs-lisp
  (use-package project-x
    ...
    :bind (
      <<keybindings-gen(table=project-x-entry-keybindings)>>
    ))
  ...
  (use-package projection
    ...
    :bind-keymap
    <<keybindings-gen(table=projection-entry-keybindings)>>)
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package mistty
    ...
    (defun mistty-send-down ()
      (interactive)
      (mistty-send-string "\e[B"))

    (defun mistty-send-up ()
      (interactive)
      (mistty-send-string "\e[A")))
...
#+begin_src emacs-lisp
  (use-package dirvish
    :ensure  (dirvish :type git :host github :repo "seblemaguer/dirvish" :branch "fix_yank")
    ...
    (defun dired-do-transfer-to-remarkable ()
      ...
      (mapc (lambda (x)
              (when (s-ends-with-p ".pdf" x t)
                ...
                (call-process "rm_import" nil nil nil x)))
            (dired-get-marked-files nil nil nil nil t))
      (message "Transfer to remarkable done"))
...
#+transclude: [[file:keytable_summary.org::file-info-entry-keybindings]]
...
#+begin_src emacs-lisp
  (use-package file-info
    ...
    :bind (
      <<keybindings-gen(table=file-info-entry-keybindings)>>
    ))
#+end_src
...
#+transclude: [[file:keytable_summary.org::elfeed-tube-entry-keybindings]]
#+transclude: [[file:keytable_summary.org::elfeed-tube-mpv-keybindings]]
...
#+begin_src emacs-lisp
  ...
  (use-package elfeed-tube-mpv
    ...
    :config
    (setq elfeed-tube-mpv--available-p (executable-find "mpv")))
#+end_src
...
:PROPERTIES:
:ID:       76dd80a4-324a-442a-8fad-51231c44c107
:END:
=Cuckoo= provides additional helpers to search the *content* of the RSS feeds.

#+transclude: [[file:keytable_summary.org::cuckoo-search-keybindings]]
...
#+BEGIN_SRC emacs-lisp
    (use-package cuckoo-search
      ...
      :bind (
        :map elfeed-search-mode-map
        <<keybindings-gen(table=cuckoo-search-keybindings)>>
      ))
#+END_SRC
** DONE Mail [100%]
...
Because I want send rich emails, I use =org-msg= which relies on the HTML exporter provided by =org= to generate HTML emails.
...
#+begin_src emacs-lisp
  (use-package org-msg
    ...
    (elpaca-after-init . org-msg-mode)
    ...
    (setq user-full-name "Sébastien Le Maguer"
          ...
          message-fill-column 130
          org-msg-enforce-css (org-msg-css-file-to-list (format "%s/third_parties/styles/org-msg.css" user-emacs-directory))
...
#+begin_src emacs-lisp
    (use-package mu4e
      :ensure (mu4e :host github
                    ...
                    :pre-build (("./autogen.sh")
                                ...
                                (make-symbolic-link (expand-file-name "./build/mu/mu")
                                                    (expand-file-name "~/environment/local/bin/mu")
                                                    'ok-if-exists))
      ...
      ;; :hook
      ;; (mu4e-update-pre . etc/imapfilter)
    ...
    Taken from https://etienne.pflieger.bzh/emacs.d/init.html."
      ...
      (defun etc/imapfilter ()
        "Run imapfilter (if available, else imply indicates that it is not available)."
        (message "Running imapfilter...")
        (with-current-buffer (get-buffer-create "*imapfilter*")
          (goto-char (point-max))
          (insert "---\n")
          (if (executable-find "imapfilter")
              (progn
                (call-process "imapfilter" nil (current-buffer) nil "-v")
                (message "Running imapfilter...done"))
            (insert "imapfilter is not available")
            (message "Running imapfilter...FAILED"))))
  ...
  (use-package mu4e-walk
    ...
    :after mu4e
    :hook
    (mu4e-compose-mode . mu4e-walk-add-keybindings-compose-mode))
#+end_src
...
#+begin_src emacs-lisp
  (use-package slack
    ...
    :commands (slack-start slack-refresh-token)
    :no-require t
...
*** DONE Ement
:PROPERTIES:
:ID:       ee21c84d-4805-4a08-940d-480cb20e4704
:END:
I use matrix a lot, so I use =ement= as a client.
...
#+BEGIN_SRC emacs-lisp
  (use-package ement
    ...
    :custom-face
    (ement-room-read-receipt-marker ((t (:height 0.1 :background "red"))))
    (ement-room-fully-read-marker ((t (:height 0.1 :background "green"))))
    ...
    (defun ement-room--no-ts-headers (&optional start-node end-node)
      "Redefinition to inhibate the header!")
    (advice-add 'ement-room--insert-ts-headers :override #'ement-room--no-ts-headers)

    (defun ement-notify--notifications-sauron (event room session)
      ...
      (pcase-let* (((cl-struct ement-event sender content) event)
        ...
        (when (and (stringp body)
                   (not (s-starts-with? "#" room-name)))
          ;; If event has no body, it was probably redacted or something, so don't notify.
          (truncate-string-to-width body 60)
          (sauron-add-event
           (intern room-name) 2 body
           (lambda () (ement-room-view room session))))))
    (advice-add 'ement-notify--notifications-notify :override #'ement-notify--notifications-sauron))
#+END_SRC
...
#+begin_src emacs-lisp
  (use-package metal-archives-shopping-list
    :ensure (:protocol ssh :type git :host github :repo "seblemaguer/metal-archives.el")
...
#+begin_src emacs-lisp
  ...
  (when (executable-find "direnv")
    (use-package ben
      :tags '("SYSTEM")
      :hook (elpaca-after-init . ben-global-mode)))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package tramp-hlo
    :tags '("SYSTEM" "SERVER" "CONNECTED")
    :disabled t ;; SLM TODO: check this when emacs-stable will move to 31
    :config
    (tramp-hlo-setup))

  (use-package tramp-rpc
    :ensure (:url "https://github.com/ArthurHeymans/emacs-tramp-rpc" :rev :newest :lisp-dir "lisp")
    :tags '("SYSTEM" "SERVER" "CONNECTED")
    :after tramp)
...
While I was using anki-editor before, it turns out that it create a lot of verbose files for nothing.
Therefore, I rely on the following process:
  1. I store vocabulary in an org-table
  2. I use =org-language-learning= to manipulate these tables:
     - generate an HTML file for practicing without using Anki
     - generate TSV file to be used as an input of a genanki script to generate an Anki deck
     - merge tables if wanted

#+transclude: [[file:keytable_summary.org::anki-entry-keybindings]]

#+begin_src emacs-lisp
  (use-package org-language-learning
    :tags '("HELPERS" "DESKTOP" "ORG")
    :ensure (:protocol ssh :host github :repo "seblemaguer/org-language-learning"))
#+end_src
...
#+transclude: [[file:keytable_summary.org::ellama-entry-keybindings]]
...
#+begin_src emacs-lisp
  (when (executable-find "ollama")
    (use-package ellama
      ...
      :bind (
        <<keybindings-gen(table=ellama-entry-keybindings)>>
      )
      ...
      (setopt ellama-provider (make-llm-ollama :chat-model "gemma4:latest"
  	                                     :embedding-model "nomic-embed-text"
  	                                     :default-chat-non-standard-params '(("num_ctx" . 8192))))
      ;; Naming new sessions with llm
      (setopt ellama-naming-provider (make-llm-ollama :chat-model  "gemma4:latest"
                                                      :embedding-model "nomic-embed-text"
...
*** DONE Weather forecast
CLOSED: [2025-12-10 Wed 18:02]
:PROPERTIES:
:ID:       e3113f27-18e4-452e-8aa9-43c0c97c1aa8
:END:
...
#+BEGIN_SRC emacs-lisp
  (use-package sparkweather
    ...
    :after calendar
    :custom
    (calendar-latitude  60.1695)
    (calendar-longitude 24.9355))
#+END_SRC

*** DONE QRCode
CLOSED: [2026-07-24 Fri 15:55]
:PROPERTIES:
:ID:       02ac32fc-13e0-42c8-815d-517433086e0d
:END:
Add helpers to support QR codes in emacs
...
I use =indicators= to show my relative position in the buffer.
Finally, I like to know which buffer I am currently using, and the package ~auto-dim-other-buffers~ provides a nice way to change the background of the focussed/unfocussed window.
...
#+begin_src emacs-lisp
  ...
  (use-package auto-dim-other-buffers
    ...
    :custom-face
    (auto-dim-other-buffers-hide ((((background light)) :foreground ,fui-light-blue :background ,fui-light-blue)
                                  (t                    :background ,fui-clouds)))
    (auto-dim-other-buffers      ((((background light)) :background ,fui-light-blue)
                                  (t                    :background ,fui-clouds)))
    :config
...
:PROPERTIES:
:ID:       39aeb578-0009-409b-b9ff-501ce76e3500
:END:
#+BEGIN_SRC emacs-lisp
...
CLOSED: [2024-11-15 Fri 13:02]
:PROPERTIES:
:ID:       e6b4da00-0385-4f2e-9ba9-5f1f75a4ab1b
:END:
#+BEGIN_SRC emacs-lisp
...
*** DONE Define the transient helper bindings
CLOSED: [2025-09-30 Tue 10:29]
:PROPERTIES:
