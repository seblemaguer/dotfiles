...
Finally, I also use =vulpea= for note taking.
...
I use it in conjunction to =prescient=:
=prescient= is responsible for the sorting;
=orderless= of the completion filtering.
...
#+begin_src elisp
  (use-package bibtex-actions
    ...
    :custom
    (bibtex-completion-bibliography '("~/work/websites/seblemaguer.github.io/publis.bib"
                                      "~/pCloudDrive/org/bibliography/")))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package multi-vterm
    :tags '("TERM")
    :init
    (defun new-vterm-and-rename (name)
      "Create a new vterm and rename it directly"
      (interactive "sName of the terminal: ")
      (multi-vterm)
      (multi-vterm-rename-buffer name))

    :bind
    (:map vterm-mode-map
          ("M-n"        . new-vterm-and-rename)
