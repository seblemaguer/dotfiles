...
#+begin_src emacs-lisp
    ...
    (use-package mu4e-thread-folding
      ...
      (defvar my-mu4e-unwanted-subject-tags
        '("[External]" "[EXTERNAL]")
        "List of tags to strip from email subject lines in mu4e headers.")

      (defun my-mu4e-strip-subject-tags (subject)
        "Strip unwanted tags (e.g. [External]) from SUBJECT string."
        (let ((case-fold-search t))  ;; Make it case-insensitive
          (dolist (tag my-mu4e-unwanted-subject-tags subject)
            (setq subject (replace-regexp-in-string
                           (concat "^" (regexp-quote tag) "[ \t]*") "" subject)))))
  ...
  e.g. \"mutt\"."
        (let* ((tinfo (mu4e-message-field msg :meta))
               (subj (my-mu4e-strip-subject-tags (mu4e-msg-field msg :subject))))
          ...
          (cl-case field
            (:subject
             (concat ;; prefix subject with a thread indicator
              ...
              (truncate-string-to-width (my-mu4e-strip-subject-tags val) 600)))
            (:thread-subject ;; if not searching threads, fall back to :subject
             (if mu4e-search-threads
                 (mu4e~headers-thread-subject msg)
               (my-mu4e-strip-subject-tags (mu4e~headers-field-value msg :subject))))
            ((:maildir :path :message-id) val)
      ...
      (defun my-mu4e-subject-formatter (msg)
        "Custom subject formatter that strips [EXTERNAL] tag."
        (my-mu4e-strip-subject-tags (mu4e-message-field msg :subject)))
