...
#+begin_src shell :results output replace :tangle ~/.bashrc
  ...
  alias msqueue='squeue -o "%.8i %.9P %.20j %.8u %.8T %.10M %.12l %.8C %.8m %.25b %.15R"'
  ...
  __conda_setup="$('$HOME/environment/local/miniconda3/bin/conda' 'shell.bash' 'hook' 2>/dev/null)"
  if [ $? -eq 0 ]; then
      eval "$__conda_setup"
  else
      if [ -f "$HOME/environment/local/miniconda3/etc/profile.d/conda.sh" ]; then
          . "$HOME/environment/local/miniconda3/etc/profile.d/conda.sh"
      else
          export PATH="$HOME/environment/local/miniconda3/bin:$PATH"
      fi
  fi
  unset __conda_setup
  # <<< conda initialize <<<
...
#+begin_src shell :results output replace :tangle ~/.zshrc
  ...
  ## Conda
  # !! Contents within this block are managed by 'conda init' !!
  __conda_setup="$('$HOME/environment/local/miniconda3/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
  if [ $? -eq 0 ]; then
      eval "$__conda_setup"
  else
      if [ -f "$HOME/environment/local/miniconda3/etc/profile.d/conda.sh" ]; then
